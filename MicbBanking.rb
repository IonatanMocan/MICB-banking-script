...
class MicbBanking
  ...
  def select_english
    @browser.li(class: ['language-item', 'en']).click
  end
  ...
  def get_accounts(html)
    ...
    @accounts = []
    nature = parsed_accounts.css('.section-title', '.h-small').text
    parsed_accounts.css('div.status-active').map do |account|
      name      = account.at_css('.name').text
      balance   = ((account.css('.amount').first.text).to_f).round(2)
  ...
  def get_transactions
    ...
    @accounts.each do |account|
      ...
      sleep 1
      unless @browser.div(class: 'operations').div(class: 'empty-message').present?
  ...
  def accounts_and_transactions
    ...
    @accounts.map do |account|
      ...
      final_hash["accounts"] << account_hash
    end
    final_hash
  end
